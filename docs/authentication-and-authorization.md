...
the domain's principal identifiers.

## Identity and sessions

- Better Auth's D1 user ID is OpenBoard's stable `PrincipalId`.
- Domain records reference `PrincipalId`, never an email address.
- One principal has one verified email address in the first version.
- Better Auth issues a secure, HttpOnly, same-origin session cookie. Browser JavaScript never holds
  an API token.
- A session expires after seven inactive days and renews while its principal remains active.
...
creates or resumes their session, and writes the submission under their `PrincipalId`.
...
OpenBoard then links the submission speaker to their `PrincipalId`; authorization no longer depends
on email matching.
...
Any verified principal can create an event and becomes its owner. Additional organizers and
...
Authentication establishes the principal. It does not establish permission to an event or resource.
...
- `public`: no principal required; limited to deliberately public data and authentication entry
  points.
...
- `submissionOwner`: the principal accountable for the requested submission.
- `submissionSpeaker`: a principal linked as a speaker to the requested submission.
...
- Any authenticated principal may create an event.
...
- Roles are additive. A principal may organize, review, and speak within one event.
...
- Reviewer procedures return an evaluation-specific projection. They do not expose the complete
  organizer-facing submission, speaker contact details, tasks, files, communications, or unreleased
  decisions.
- Whether a reviewer projection includes speaker names is evaluation policy, not a broader access
  grant.
...
- A speaker may access their own personal tasks and files.
- Speakers linked to a submission may access its shared tasks and files.
- Organizers may access personal and shared resources where required to run their event.
...
- A recipient may access only communications addressed to their principal.
- Event membership alone does not grant access to another recipient's communication.

## State owned in D1

Better Auth owns its user, session, account, and verification records in OpenBoard's D1 database.
OpenBoard owns the authorization state that gives those principals meaning inside the product:

- event ownership and organizer or reviewer membership;
...
OpenBoard records event creation, ownership transfer, invitation acceptance or revocation,
membership changes, speaker-account linking, publication, and denied mutation attempts. Ordinary
...
- Multiple verified emails per principal.
